-- ============================================================================
-- 20260806172306_unify_colmex_snapshot_chart_algorithm.sql
--
-- בעיה: תיקי Colmex כתבו portfolio_value = equity (כולל unrealized),
-- בזמן ש-buildHistoricalPortfolioSeriesFromSnapshots מוסיף unrealized שוב
-- מה-trades + מחירי Yahoo — אותה נוסחה כמו תיקים ידניים.
--
-- תיקון:
-- 1. הסר overwrite ל-equity מ-sync_broker_positions_to_trades
-- 2. snapshot נשאר net_deposits + realized (upsert_daily_snapshot)
-- 3. recalc לכל תיקי colmex_pro מהפעילות הראשונה עד היום
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 1. sync_broker_positions_to_trades — בלי overwrite ל-equity
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  -- ---- בניית CLOSED trades מ-executions ----
  FOR v_closed IN
    SELECT
      be.position_id,
      COALESCE(
  ...
  -- אותו אלגוריתם כמו תיקים ידניים: net_deposits + realized (unrealized בלקוח)
...
COMMENT ON FUNCTION public.sync_broker_positions_to_trades(UUID) IS
  'מסנכרן broker_positions/executions → trades, מעדכן available_cash, ו-snapshot '
  'בנוסחת net_deposits+realized (זהה לתיקים ידניים; unrealized מחושב בלקוח).';

GRANT EXECUTE ON FUNCTION public.sync_broker_positions_to_trades(UUID) TO authenticated, service_role;

-- ----------------------------------------------------------------------------
-- 2. Backfill: מחק snapshots של Colmex ובנה מחדש בנוסחה הסטנדרטית
-- ----------------------------------------------------------------------------
DO $$
DECLARE
  r RECORD;
...
BEGIN
  FOR r IN
    SELECT id FROM public.portfolios WHERE source = 'colmex_pro'
  LOOP
    SELECT LEAST(
      (SELECT MIN(date::date) FROM public.portfolio_transactions WHERE portfolio_id = r.id),
      (SELECT MIN(public.local_date_key(entry_date, 'UTC')) FROM public.trades WHERE portfolio_id = r.id),
      (SELECT MIN(public.local_date_key(exit_date, 'UTC'))
         FROM public.trades
        WHERE portfolio_id = r.id AND exit_date IS NOT NULL)
    ) INTO v_from;

    DELETE FROM public.daily_portfolio_snapshots WHERE portfolio_id = r.id;

    IF v_from IS NULL THEN
      PERFORM public.upsert_daily_snapshot(r.id, CURRENT_DATE);
    ELSE
      PERFORM public.range_recalc_snapshots(r.id, v_from, CURRENT_DATE);
    END IF;
  END LOOP;
END;
$$;
