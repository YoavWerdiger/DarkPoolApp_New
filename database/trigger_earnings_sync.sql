-- ============================================
-- פונקציה SQL להפעלת Earnings Sync
-- ============================================
-- 
-- פונקציה זו קוראת ל-Edge Function daily-earnings-sync-simple
-- עם טווח תאריכים של 3 חודשים אחורה + שנה קדימה

-- יצירת פונקציה
CREATE OR REPLACE FUNCTION trigger_earnings_sync(
...
BEGIN
  ...
  IF p_date_to IS NULL THEN
    -- שנה קדימה - אבל זה לוקח יותר מדי זמן, אז נשתמש ב-3 חודשים קדימה
    to_date := (today_est + INTERVAL '3 months')::DATE;
  ELSE
    to_date := p_date_to;
  END IF;
  ...
  RETURN QUERY SELECT 
    TRUE as success,
    'Earnings sync triggered successfully. Check logs in Supabase Dashboard.' as message,
    http_response_id,
    from_date_str,
    to_date_str;
...
-- המתן 2-5 דקות ואז בדוק: SELECT COUNT(*) FROM earnings_calendar;
...
-- SELECT * FROM trigger_earnings_sync('2025-01-01'::DATE, '2025-12-31'::DATE);
