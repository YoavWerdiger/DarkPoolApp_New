-- 🔔 שליחה ישירה למכשיר הפיזי
-- ===========================================
-- Token: ExponentPushToken[DOalseKrRSRTo2aoJEvdmE]
-- Device: M2102J20SG (מכשיר פיזי)
-- ID: e058127a-c026-48df-a355-1cd778410662

-- שלב 1: בדוק שהטוקן פעיל
SELECT 
  '📱 בדיקת טוקן מכשיר פיזי' as check_name,
  dt.id,
  dt.user_id,
  dt.expo_push_token,
  dt.device_id,
  ...
  dt.updated_at,
  CASE 
    WHEN dt.is_active = true THEN '✅ פעיל'
    ELSE '❌ לא פעיל'
  END as status
...
-- שלב 2: הוספת התראה ל-pending_notifications
INSERT INTO pending_notifications (
  ...
  '🔔 בדיקת DarkPool - מכשיר פיזי - ' || TO_CHAR(NOW(), 'HH24:MI:SS'),
  'זוהי התראה למכשיר הפיזי שלך (M2102J20SG) - האם אתה רואה "DarkPool" בשם האפליקציה? 🎉 בדוק שהלוגו של DarkPool מופיע!',
  'test',
  jsonb_build_object(
    'type', 'test',
    'timestamp', NOW()::text,
    'test', true,
    'message', 'התראה נשלחה למכשיר פיזי: M2102J20SG',
    'device', 'M2102J20SG',
    'token', 'ExponentPushToken[DOalseKrRSRTo2aoJEvdmE]',
    'appName', 'DarkPool'
  ),
...
BEGIN
  ...
  RAISE NOTICE '';
  RAISE NOTICE '📱 עכשיו בדוק את המכשיר הפיזי:';
