...
| 2.3 | **Unit test scaffolding.** | DONE | `jest` + `jest-expo/node` preset. 32 tests across chat validation (XSS, length, rate-limit isolation per modality) and portfolio FIFO / XIRR. Runs in `npm run test:ci` from CI. |
...
| # | Item |
|---|------|
| 6.1 | Wrap `renderMessage` in `useCallback`; fix `ChatMessage` memo comparator to include `isHighlighted` (`CHAT_SYSTEM_AUDIT.md` § 1). |
| 6.2 | Throttle outbound `setTypingStatus` to ≥ 1Hz. |
| 6.3 | Migrate `chat_messages` `FlatList` → `@shopify/flash-list` (already a dependency). |
| 6.4 | Persist React Query cache via `@tanstack/query-async-storage-persister` for offline-first chat opens. |
| 6.5 | Audit bundle with `npx expo export --dump-sourcemap` and check unused locale data from `date-fns`. |
